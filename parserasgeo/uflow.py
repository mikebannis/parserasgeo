...
class UnsteadyFlow(object):
    ...
    def __init__(self, filename):
        ...
        self.uflow_list = []  # list of strings and line objects created from file
        ...
        with open(filename, 'rt') as infile:
            while True:
                ...
                elif Inflow.test(line):
                    inflow = Inflow()
                    inflow.import_geo(line, infile)
                    self.uflow_list.append(inflow)
...
class Inflow(object):
    ...
    @staticmethod
    def test(line):
        if line.split('=')[0] == 'Lateral Inflow Hydrograph':
            return True
        else:
            return False
    ...
    def __str__(self):
        s = 'Lateral Inflow Hydrograph= ' + str(self.number) + ' \n'
        return s
...
class Header(object):
    ...
    def import_geo(self, line, infile):
        ...
        self.location = self._parts[2].strip()
...
def main():
    filename = '../geos/test.u01'
    outfile = '../geos/out.u01'

    uf = UnsteadyFlow(filename)
    print(uf.uflow_list)
    uf.export(outfile)
